...
****************************************************************************
****
***  TSB712 Spice macromodel subckt 
***
...
****                    |   |      |       |      |     
****                    |   |      |       |      |  
.SUBCKT TSB712    VP  VM  VCCP VCCN  VS
    LOUT VZOUT 0 {LOUT}
...
+VALUE={IF(V(val_vdep_sink)<=0 , 0 , V(val_vdep_sink))}
    E64 ALPHA_DESAT_TIME 0 POLY(1) VCCP VCCN 0.10048387096774188
+-0.002096774193548386
...
+V(VOH_noRL) }
    E_VDEP_SOURCE_1 VAL_VDEP_SOURCE 0 VALUE={ 249.5 -5000*I(VreadIo)}
...
+VALUE={IF(V(val_vdep_source)>=0, 0, V(val_vdep_source))}
    ...
    E_RO1_VOH RO1_VOH 0 POLY(1) VCCP VCCN 86.12903225806448
+-1.2258064516129028
    ...
    E_RO1_VOL RO1_VOL 0 POLY(1) VCCP VCCN 86.12903225806448
+-1.2258064516129028
    E_ICCSAT_HIGH ICC_OUT_HIGH 0 VALUE={0}
    E_SR_VCC_MODULATION VOUT_DIFF__SR_VCC 0 VALUE={V(Vout_diff)*(
+0.9535483870967739 + 0.0012903225806451615*V(Vccp,Vccn) )}
    EZOUT VB_3 NET0150
...
+Zout_Iout_coeff_MIN)*exp(-abs(I(VreadIo)/Iout_dc_tau__sink)) ) )}
    EZOUT_VCC_COEFF VZOUT_VCC_COEFF 0 POLY(1) VCCP VCCN 3.3225806451612887
+-0.06451612903225803
...
+V(Icc_out_high), 0)}
    G_ICC VCCP VCCN POLY(1) VCCP VCCN 0.0013054838709677418
+1.2903225806451616E-5
...
*******************************************************************************
*
* MODELS/SUBCKTS and PARAMS used by TSB712 subckt:
*
.PARAM VCC_MIN=1.5
.PARAM RINCM=2.3482E+09
.PARAM CINCM=2.8629E-12
.PARAM RIN_CM_VM={2*RINCM}
.PARAM RIN_CM_VP={2*RINCM}
.PARAM CIN_CM_VM={CINCM/2}
.PARAM CIN_CM_VP={CINCM/2}
.PARAM RINDIFF=1.3985E+06
.PARAM CINDIFF=5.9157E-12
